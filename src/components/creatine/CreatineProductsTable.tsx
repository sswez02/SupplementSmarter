  ...
  return (
    <div className='flex flex-wrap gap-1.5'>
      {first.map((f) => (
        <span
          key={f}
          className='px-2 py-0.5 text-xs rounded-full bg-black/0.04 text-black/70'
          title={f}
        >
          {f}
        </span>
      ...
      {extra > 0 && (
        <span className='px-2 py-0.5 text-xs rounded-full bg-black/0.04 text-black/50'>
          +{extra} more
        </span>
      )}
    </div>
...
export default function CreatineTable() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error('Error fetching /api/creatine:', err);
      }
    })();
  ...
  const filteredRows = rows.filter((row) => {
    ...
    return brandMatch && weightMatch && searchMatch;
  });
              ...
              <tbody className='text-black text-sm'>
                {displayRows.map((row) => (
                  <tr
                    ...
                    onClick={() => row.id && navigate(`/creatine/${row.id}`)}
