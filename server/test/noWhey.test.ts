...
import { scrapeNoWheyProtein } from '@/scrapers/protein/noWhey_Protein.js';
import { scrapeNoWheyCreatine } from '@/scrapers/creatine/noWhey_Creatine.js';
import { CURRENCIES, RETAILERS, type Product } from '@/scrapers/common/types.js';

describe('No Whey scraper', () => {
  it('returns protein products within time budget and with valid fields', async () => {
    const { products, errors } = await scrapeNoWheyProtein();
    ...
    for (const p of products as Product[]) {
      ...
      if ('flavours' in p && p.flavours !== undefined) {
        ...
        // Allow empty array, but if non-empty they must be non-empty strings
  ...
  it('returns creatine products within time budget and with valid fields', async () => {
    const { products, errors } = await scrapeNoWheyCreatine();
