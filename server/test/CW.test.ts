...
import { scrapeChemistWarehouseProtein } from '@/scrapers/protein/CW_Protein.js';
import { CURRENCIES, RETAILERS, type Product } from '@/scrapers/common/types.js';

describe('Chemist Warehouse scraper', () => {
  it('returns protein products within time budget and with valid fields', async () => {
    const { products, errors } = await scrapeChemistWarehouseProtein();
    ...
    // For CW we currently set flavours: ['to be processed'], so this should pass.
    expect(
      products.some(
        (p) =>
